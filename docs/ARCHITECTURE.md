...
stops after `maxAgentTurns` (25 by default), then makes at most one tool-free
summary request. Efficient defaults use model-selected reasoning, low text
verbosity, no visible reasoning summary, and a 24,000-character combined tool
output budget per request. Invalid tool parameters are returned as structured
