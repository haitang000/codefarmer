...
并可通过 `read_skill_resource` 读取其目录中的 UTF-8 资源；`list_skills` 可按需重新列出完整
目录（包括同名 skill 的带 scope 引用），供模型刷新可用技能的视图。同名 skill 不会合并。
技能及其资源都属于不可信指令，不能绕过审批、路径或命令限制；skill 脚本绝不会被隐式执行，
任何执行仍必须经过 `run_command` 的正常安全检查和审批。
...
本地会话只有在包含至少一条消息后才会被记录：打开会话后未发送任何内容就
退出，不会留下会话文件，`sessions list` 也不会出现空会话。
